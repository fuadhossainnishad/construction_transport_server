...
import (
	...
	"construction_transport_server/internal/job/domain"
	"context"
...
type BookingRepository interface {
	...
	AddStatusTimeline(ctx context.Context, bookingID int64, status, notes string) error
	GetTimeline(ctx context.Context, bookingID int64) ([]domain.JobTimeline, error)
}
...
func (r *bookingRepo) AddStatusTimeline(ctx context.Context, bookingID int64, status, notes string) error {
	_, err := r.db.Exec(ctx, `INSERT INTO job_status_updates (booking_id, status, notes) VALUES ($1, $2, $3)`, bookingID, status, notes)
	return err
}

func (r *bookingRepo) GetTimeline(ctx context.Context, bookingID int64) ([]domain.JobTimeline, error) {
	rows, err := r.db.Query(ctx, `SELECT status, notes, created_at FROM job_status_updates WHERE booking_id = $1 ORDER BY created_at ASC`, bookingID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var t domain.JobTimeline
		rows.Scan(&t.Status, &t.Notes, &t.CreatedAt)
