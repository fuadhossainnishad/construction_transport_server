...
func (r *accountRepo) CreateAccount(profile *domain.UserProfile) error {
    ...
    return r.db.QueryRow(context.Background(), query, 
        profile.UserId, profile.Name, profile.PhoneNumber, profile.ProfileImage, profile.Location).Scan(&profile.ID)
}
...
func (r *accountRepo) UpdateAccount(profile *domain.UserProfile) error {
    query := `UPDATE profiles SET full_name = $1, phone_number = $2, profile_image = $3, location = $4, updated_at = NOW() 
              WHERE user_id = $5`
    _, err := r.db.Exec(context.Background(), query, 
        profile.Name, profile.PhoneNumber, profile.ProfileImage, profile.Location, profile.UserId)
    return err
}